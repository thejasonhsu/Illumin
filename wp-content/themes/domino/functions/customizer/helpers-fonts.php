...
if ( ! function_exists( 'domino_font_get_relative_sizes' ) ) :
    /**
     ...
     */
    function domino_font_get_relative_sizes() {
        // Relative font sizes
        return apply_filters( 'domino_font_relative_size', array(

        ) );
    }
endif;

if ( ! function_exists( 'domino_css_fonts' ) ) :
    /**
     * Build the CSS rules for the custom fonts
     *
     * @return void
     */
    function domino_css_fonts() {
        // Get relative sizes
        $percent = domino_font_get_relative_sizes();
        ...
        $declarations = domino_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            domino_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
         ...
         */
        $element = 'nav';
        $selectors = array( '.main-navbar .navbar-nav a' );
        $declarations = domino_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            domino_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
        ...
        $selectors = array( '.slides li h3', '.slides li h3 a' );
        $declarations = domino_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            domino_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
...
add_action( 'domino_css', 'domino_css_fonts' );

if ( ! function_exists( 'domino_parse_font_properties' ) ) :
    /**
     ...
     */
    function domino_parse_font_properties( $element ) {
        // css_property => sanitize_callback
        $properties = apply_filters( 'domino_css_font_properties', array(
            'font-family'	=> 'domino_get_font_stack',
            'font-size'		=> 'absint',
        ), $element );

        $declarations = array();
        foreach ( $properties as $property => $callback ) {
            $value = get_theme_mod( $property . '-' . $element, domino_get_default( $property . '-' . $element ) );
            if ( false !== $value && $value !== domino_get_default( $property . '-' . $element ) ) {
                ...
                if ( 'font-size' === $property ) {
                    ...
                    // $declarations[$property . '-rem'] = domino_convert_px_to_rem( $sanitized_value ) . 'rem';
                } else {
...
if ( ! function_exists( 'domino_get_font_stack' ) ) :
    /**
     ...
     */
    function domino_get_font_stack( $font ) {
        $all_fonts = domino_get_all_fonts();

        // Sanitize font choice
        $font = domino_sanitize_font_choice( $font );

        // Standard font
        if ( isset( $all_fonts[ $font ]['stack'] ) && ! empty( $all_fonts[ $font ]['stack'] ) ) {
            $stack = $all_fonts[ $font ]['stack'];
        } elseif ( in_array( $font, domino_all_font_choices() ) ) {
         ...
         */
        return apply_filters( 'domino_font_stack', $stack, $font );
    }
endif;

if ( ! function_exists( 'domino_get_relative_font_size' ) ) :
    /**
     ...
     */
    function domino_get_relative_font_size( $value, $percentage ) {
        return round( (float) $value * ( $percentage / 100 ) );
    }
endif;

if ( ! function_exists( 'domino_convert_px_to_rem' ) ) :
    /**
     ...
     * @return float             The converted value.
     */
    function domino_convert_px_to_rem( $px, $base = 0 ) {
        return (float) $px / 10;
    }
endif;

if ( ! function_exists( 'domino_get_font_property_option_keys' ) ) :
    /**
     ...
     */
    function domino_get_font_property_option_keys( $property ) {
        $all_keys = array_keys( domino_option_defaults() );
...
if ( ! function_exists( 'domino_get_google_font_uri' ) ) :
    ...
    function domino_get_google_font_uri() {
        ...
        foreach ( $font_keys as $key ) {
            $fonts[] = get_theme_mod( $key, domino_get_default( $key ) );
        }

        // De-dupe the fonts
        $fonts         = array_unique( $fonts );
        $allowed_fonts = domino_get_google_fonts();
        $family        = array();
        ...
        foreach ( $fonts as $font ) {
            ...
            if ( array_key_exists( $font, $allowed_fonts ) ) {
                ...
                $family[] = urlencode( $font . ':' . join( ',', domino_choose_google_font_variants( $font, $allowed_fonts[ $font ]['variants'] ) ) );
            }
        }
        ...
        $subset = get_theme_mod( 'font-subset', domino_get_default( 'font-subset' ) );

        if ( 'all' === $subset ) {
            $subsets_available = domino_get_google_font_subsets();
...
if ( ! function_exists( 'domino_choose_google_font_variants' ) ) :
    /**
     ...
     */
    function domino_choose_google_font_variants( $font, $variants = array() ) {
        $chosen_variants = array();
        if ( empty( $variants ) ) {
            $fonts = domino_get_google_fonts();
         ...
         * @param array     $variants    The variants for the font.
         */
        return apply_filters( 'domino_font_variants', array_unique( $chosen_variants ), $font, $variants );
    }
endif;

if ( ! function_exists( 'domino_sanitize_font_subset' ) ) :
    /**
     ...
     */
    function domino_sanitize_font_subset( $value ) {
        if ( ! array_key_exists( $value, domino_get_google_font_subsets() ) ) {
            $value = 'latin';
        }
         ...
         */
        return apply_filters( 'domino_sanitize_font_subset', $value );
    }
endif;

if ( ! function_exists( 'domino_get_google_font_subsets' ) ) :
    /**
     ...
     */
    function domino_get_google_font_subsets() {
        /**
         ...
         */
        return apply_filters( 'domino_get_google_font_subsets', array(
            'all'          => __( 'All', 'wpzoom' ),
...
if ( ! function_exists( 'domino_sanitize_font_choice' ) ) :
    /**
     ...
     */
    function domino_sanitize_font_choice( $value ) {
        if ( ! is_string( $value ) ) {
            // The array key is not a string, so the chosen option is not a real choice
            return '';
        } else if ( array_key_exists( $value, domino_all_font_choices() ) ) {
            return $value;
        } else {
            return '';
        }
...
if ( ! function_exists( 'domino_font_choices_placeholder' ) ) :
    /**
...
if ( ! function_exists( 'domino_all_font_choices' ) ) :
    ...
    function domino_all_font_choices() {
         ...
         */
        return apply_filters( 'domino_all_font_choices', $choices );
    }
endif;

if ( ! function_exists( 'domino_all_font_choices_js' ) ) :
    /**
     * Compile the font choices for better handling as a JSON object
     *
     * @return array
     */
    function domino_all_font_choices_js() {
        $fonts   = domino_get_all_fonts();
        $choices = array();
...
if ( ! function_exists( 'domino_get_all_fonts' ) ) :
    /**
     * Compile font options from different sources.
     *
     * @return array    All available fonts.
     */
    function domino_get_all_fonts() {
        $heading1       = array( 1 => array( 'label' => sprintf( '--- %s ---', __( 'Standard Fonts', 'wpzoom' ) ) ) );
        $standard_fonts = domino_get_standard_fonts();
        $heading2       = array( 2 => array( 'label' => sprintf( '--- %s ---', __( 'Google Fonts', 'wpzoom' ) ) ) );
        $google_fonts   = domino_get_google_fonts();
         ...
         */
        return apply_filters( 'domino_all_fonts', array_merge( $heading1, $standard_fonts, $heading2, $google_fonts ) );
    }
endif;

if ( ! function_exists( 'domino_get_standard_fonts' ) ) :
    /**
     * Return an array of standard websafe fonts.
     *
     * @return array    Standard websafe fonts.
     */
    function domino_get_standard_fonts() {
        /**
         * Allow for developers to modify the standard fonts.
         *
         * @param array    $fonts    The list of standard fonts.
         */
        return apply_filters( 'domino_get_standard_fonts', array(
            'serif' => array(
...
if ( ! function_exists( 'domino_get_google_fonts' ) ) :
    /**
     * Return an array of all available Google Fonts.
     *
     * @return array    All Google Fonts.
     */
    function domino_get_google_fonts() {
        /**
         ...
         */
        return apply_filters( 'domino_get_google_fonts', array(
            'ABeeZee' => array(
                'label'    => 'ABeeZee',
                'variants' => array(
                    'regular',
